...
bool RobStrideDriver::initialize(const hardware_interface::HardwareInfo & info)
{
  try {
    auto configuration = parse_driver_configuration(info, logger_);
...
bool RobStrideDriver::validate_command_switch(
  const std::vector<std::string> & start, const std::vector<std::string> & stop)
{
  ...
  if (!validate_command_mode_switch(current_states, start, stop, &validation_error)) {
    RCLCPP_ERROR(logger_, "Command mode switch rejected: %s", validation_error.c_str());
    return false;
  }
  for (const auto & key : start) {
    for (const auto & joint : joints_) {
      if (key == joint.name + "/position" && !joint.feedback_status.received) {
        RCLCPP_ERROR(
          logger_, "Cannot start position command interface for '%s' before first feedback",
          joint.name.c_str());
        return false;
      }
    }
...
void RobStrideDriver::apply_command_switch(
  const std::vector<std::string> & start, const std::vector<std::string> & stop)
{
  std::lock_guard<std::mutex> lock(state_mutex_);
